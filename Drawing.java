...
/**
 *
 * @author Emile Lambolais
 */
public class Drawing extends JPanel implements MouseInputListener {

    private int widthMD, heightMD;
    private Color color;
    private String nextFigure;
    private Point origin, end, origin2;
    private ArrayList<Figure> listOfFigure = new ArrayList<Figure>();
    private ArrayList<Figure> listDragged = new ArrayList<Figure>();
    private ArrayList<Figure> listRestore = new ArrayList<Figure>();

    /**
     * Constructeur : permet de créer une nouvelle zone de dessin
     */
    public Drawing() {
     ...
     *
     * @param e : Evènement de de souris (clic).
     */
    @Override
     ...
     *
     * @param e : Evènement de de souris (déplacement).
     */
    @Override
    public void mouseReleased(java.awt.event.MouseEvent e) {
        ...
        if ((end.getX() < origin.getX()) && (end.getY() > origin.getY())) { // Dans le cas ou l'abscisse d'arrivée < que celle de départ, mais que l'ordonnée d'arrivée > à celle de départ => création d'un point d'origine prenant l'abscisse d'arrivée et l'ordonnée de départ
            origin = new Point(end.getX(), origin.getY()); 
        } 
        if ((end.getX() < origin.getX()) && (end.getY() < origin.getY())) { // Dans le cas ou l'abscisse d'arrivée < que celle de départ, mais que l'ordonnée d'arrivée < à celle de départ => le point d'arrivée devient le point d'origine
            origin = end;
        } 
        if ((end.getX() > origin.getX()) && (end.getY() < origin.getY())) { // Dans le cas ou l'abscisse d'arrivée > que celle de départ, mais que l'ordonnée d'arrivée < à celle de départ => création d'un point d'origine prenant l'abscisse de départ et l'ordonnée d'arrivée
     ...
     *
     * @param e : Evènement de déplacement de la souris
     */
    @Override
    public void mouseDragged(java.awt.event.MouseEvent e) {
        ...
        // Création des différents cas possibles (sens d'affichage)
        if ((e.getX() < origin.getX()) && (e.getY() < origin.getY())) { // Même méthode de raisonnement que dans le mouseRealesed
            origin2 = new Point(e.getX(), e.getY());
        }
        if ((e.getX() > origin.getX()) && (e.getY() > origin.getY())) {
            origin2 = origin;
        }
        if ((e.getX() < origin.getX()) && (e.getY() > origin.getY())) {
            origin2 = new Point(e.getX(), origin.getY());
        }
        if ((e.getX() > origin.getX()) && (e.getY() < origin.getY())) {
            origin2 = new Point(origin.getX(), e.getY());
        }
